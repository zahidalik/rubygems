class Enrollment < ApplicationRecord
  ...
  validates_uniqueness_of :course_id, scope: :user_id # course can not have same user enrolled twice
  ...
  scope :latest_good_reviews, -> { order(review: :desc, created_at: :desc).limit(3) }
