class CoursesController < ApplicationController
  ...
  def index
    # if params[:title]
    #   @courses = Course.where('title ILIKE ?', "%" + params[:title] + "%") # case sensitive because of 'I' before LIKE.
    # else
  ...
  def pending_review
    ...
    @ransack_courses = Course.joins(:enrollments).merge(Enrollment.pending_review.where(enrollments: { user: current_user})).ransack(params[:courses_search], search_key: :courses_search)
    @pagy, @courses = pagy(@ransack_courses.result.includes(:user))
    render 'index'
  end
  ...
  def destroy
    ...
    if @course.destroy
      ...
      redirect_to @course, alert: 'Course has got enrollments. Cannot be destroyed.'
    end
  end
